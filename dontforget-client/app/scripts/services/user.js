 ...
 */
angular.module('dontforgetApp').service('User', ['restRequest', function User(restRequest) {
	this.create = function (pLogin, pPassword, pEmail, pSuccessCallback) {
		return restRequest.post({
			...
			errorsCodes : {
				...
				'409' : function () {
					alert("Le pseudonyme " + pLogin + " est déjà utilisé.");//TODO TR
				},
				'default' : function (pData) {
					alert("Impossible de créer l'utilisateur. Erreur inconnue : " + pData);//TODO TR
				}
			}
		});
	};
